...
restart:
    ...
    //pass = "noneyet\0";
    // JDB DEBUG SPIT THE PASSWORD OUT SOMEWHERE:
    printf("JDB: password: '%s'\n", pass);
    alarm(0);
    save_errno = errno;
...
{
    printf("getln called. feedback: %i\n", feedback);
    size_t left = bufsiz;
    ssize_t nr = -1;
    char *cp = buf;
    char c = '\0';
    ...
    if (left == 0) {
	printf("something wrong with left??\n");
	errno = EINVAL;
	...
	if (c == '\0') {
	    ...
	    printf("Read %i chars: %c\n", nr, c);
	}

	if (nr != 1 || c == '\n' || c == '\r') {
	    printf("Hit end! Breaking!\n");
	    break;
	}
	if (feedback) {
	    printf("Uhoh. something with the feedback is probably messing up the input buffer\n");
	    if (c == term_kill) {
		printf("Termkill hit\n");
		while (cp > buf) {
		    if (write(fd, "\b \b", 3) == -1)
    ...
    *cp = '\0';
    if (feedback) {
	printf("feedback is causing erasing the stars\n");
	/* erase stars */
	while (cp > buf) {
	    if (write(fd, "\b \b", 3) == -1)
		break;
